...
17. **Realtime notifications (SSE Parte 1):** in-process `RealtimeHub` fans named events to independent SSE connections on `GET /api/realtime/notifications`. `rfp_ticket_created` publishes at ticket create (`status=analizando`); optional `sla_threshold_alert` fires from telemetry report when fulfillment &lt; 90%. Clients reconnect with exponential backoff, `last_event_id` replay, and REST reconcile — no LLM in this layer. WebSockets deferred to Parte 2.
18. **Security baseline (NIST CSF + OWASP Top 10):** inventory/suppliers reads require JWT; `POST/GET /users` is admin-only (`/auth/register` remains the public signup). `POST /api/knowledge/ask` has an in-process per-user rate limit (`TRACKFLOW_KNOWLEDGE_ASK_RATE_LIMIT`). MCP `MCP_AUTH_ALLOW_DEV_TOKEN` defaults to off; production rejects the placeholder `MCP_AUTH_JWT_SECRET`. The CX agent mints MCP tokens with `incidents:read` + `inventory:read` only. Compose binds Redis/Qdrant/MCP/Flower to `127.0.0.1`; API/MCP/UI images run as non-root. Reports: `docs/cibersecurity/NIST_REPORT.md`, `OWASP_TOP10_AUDIT.md`, `HARDENING.md`.
