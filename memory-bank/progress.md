...
- LangGraph knowledge agent Part 1 delivered: minimal `AgentState`, single-responsibility nodes (`receive_question`, `retrieve`, `generate_answer` / `generate_no_context`, `abort_invalid`), conditional edges, `MemorySaver` checkpointing, queryable traces via `GET /api/knowledge/runs/{run_id}`, Milestone 7 functions reused through `data/pipelines/rag`, and runnable evals in `tests/pipelines/test_rag_agent_evals.py`.
